...
class MinimaxPlayer2:
    ...
    def maxi_move(self, curr_state, state):
                # state.current.get_color(self)
        ## print("legalsmaxi: " + str(actions(state[0])))
        # legals = actions(state)[0]
        # scores = actions(state)[1]
        best_move = -9999999
        self.current_depth += 1
        ...
        print("Depthest: " + str(self.depth))
        if self.current_depth > self.depth:
            print("1")
            return state.utility, self.move
        for tmove in curr_state.actions(state)[0]:
            print("tmove maxi: " + str(tmove))
            state2, self.move = self.mini_move(result(state, tmove))
            print("Current State array: " + str(state2.board_array))
            print("State2: " + str(state2))
            if state2.utility > best_move:
                state.utility, self.move = state2.utility, tmove
        print("2")
    ...
    def mini_move(self, state):
        ...
        if self.current_depth > self.depth:
            print("3")
            return state.utility, self.move
        for tmove in actions(state)[0]:
            print("tmove mini: " + str(tmove))
            state2, self.move = self.maxi_move(result(state, tmove))
            print("State2: " + str(state2))
    ...
    def make_move(self, state):
        ...
        while curr_move is None:
            ...
            #     print("Here!")
            #     state, self.move = self.maxi_move(state)
            #     print(self.move)
            ...
            if not scores:
                ...
                if move == SKIP and SKIP in legals:
                    return move
            best_utility, self.move = MinimaxPlayer.maxi_move(curr_state, state)
    ...
    def maxi_move(self, state):
        ...
        if self.current_depth <= self.depth:
            for a in legals:
                ...
                print("a-" + str(a))
                self.v2.utility, a2 = self.mini_move(result(state, a))
                print("v2: " + str(self.v2.utility))
                if self.v2.utility > v:
                    print("v2: " + str(self.v2.utility))
                    print("v: " + str(v))
                    v, move = self.v2.utility, a
        return v, move
    ...
    def mini_move(self, state):
        ...
        if self.current_depth <= self.depth:
            for a in legals:
                print("legals: " + str(legals))
                print("a-" + str(a))
                self.v2.utility, a2 = self.maxi_move(result(state, a))
                print("v2: " + str(self.v2.utility))
                if self.v2.utility < v:
                    v, move = self.v2.utility, a
            print("4")
    ...
    def make_move(self, state):
        ...
        while curr_move is None:
            ...
            if not scores:
                ...
                if move == SKIP and SKIP in legals:
                    return move
            best_utility, curr_move = MinimaxPlayer.maxi_move(self, state)
            print("BEST UTILITY: " + str(best_utility))
...
def result(state, action):
    ...
    newstate = OthelloState(state.other, state.current, copy.deepcopy(state.board_array), state.utility)
...
def play_game(p1=None, p2=MinimaxPlayer(WHITE, 8)):
